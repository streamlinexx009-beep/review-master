-- ReviewMaster secure assessment RPC result payloads
-- Date: 2026-06-22
-- Purpose:
--   Replace secure submit RPCs so they return server-calculated result payloads.
--   The Flutter app uses these values to show accurate results even when
--   student-safe question RPCs omit correct_answer.

begin;

drop function if exists public.submit_exam_attempt_secure(uuid, jsonb);
...
begin
  ...
  where e.id = p_exam_id
    and (e.is_published = true or e.created_by = v_user_id or private.is_instructor_or_admin())
  limit 1;
...
drop function if exists public.submit_practice_attempt_secure(uuid, jsonb);
create function public.submit_practice_attempt_secure(
  p_topic_id uuid,
  p_answers jsonb
)
returns jsonb
...
drop function if exists public.submit_topic_exam_attempt_secure(uuid, jsonb);
create function public.submit_topic_exam_attempt_secure(
  p_exam_id uuid,
  p_answers jsonb
)
returns jsonb
