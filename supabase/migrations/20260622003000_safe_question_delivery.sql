-- ReviewMaster student-safe question delivery and result-returning scoring RPCs
-- Date: 2026-06-22
-- Purpose:
--   1. Let student-facing screens load questions without receiving correct_answer.
--   2. Return server-calculated attempt results after submission.
--   3. Keep staff/raw-table access unchanged for authoring and review screens.
...
create or replace function public.get_practice_questions_safe(p_topic_id uuid, p_limit integer default 10)
returns table (
  id uuid,
  topic_id uuid,
  question text,
...
create or replace function public.submit_exam_attempt_secure_result(
  p_exam_id uuid,
  p_answers jsonb
)
returns jsonb
...
begin
  ...
  insert into public.exam_attempt_answers (attempt_id, question_id, selected_answer, correct_answer, is_correct)
  select
    v_attempt_id,
    q.id,
...
revoke all on function public.submit_exam_attempt_secure_result(uuid, jsonb) from public, anon;
grant execute on function public.submit_exam_attempt_secure_result(uuid, jsonb) to authenticated;

create or replace function public.submit_practice_attempt_secure_result(
  p_topic_id uuid,
  p_answers jsonb
)
returns jsonb
...
revoke all on function public.submit_practice_attempt_secure_result(uuid, jsonb) from public, anon;
grant execute on function public.submit_practice_attempt_secure_result(uuid, jsonb) to authenticated;

create or replace function public.submit_topic_exam_attempt_secure_result(
  p_exam_id uuid,
  p_answers jsonb
)
returns jsonb
...
begin
  ...
  select te.topic_id into v_topic_id
  from public.topic_exams te
  where te.id = p_exam_id
    and (te.status = 'published' or private.is_instructor_or_admin())
  limit 1;
  ...
  insert into public.topic_exam_attempt_answers (attempt_id, question_id, selected_answer, correct_answer, is_correct)
  select
    v_attempt_id,
    q.id,
    coalesce(sa.selected_answer, ''),
...
revoke all on function public.submit_topic_exam_attempt_secure_result(uuid, jsonb) from public, anon;
grant execute on function public.submit_topic_exam_attempt_secure_result(uuid, jsonb) to authenticated;
