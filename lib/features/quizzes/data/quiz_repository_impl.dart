...
class QuizRepositoryImpl implements QuizRepository {
  ...
  final TopicMasteryService _topicMasteryService = TopicMasteryService();

  QuizRepositoryImpl(this.client);

  @override
  Future<List<QuizModel>> getQuizzes() async {
    final data = await client.from('quizzes').select().order('created_at');

    return data.map<QuizModel>((e) => QuizModel.fromMap(e)).toList();
  }
  ...
  Future<Map<String, dynamic>> submitQuiz({
    ...
    await _topicMasteryService.updateQuizMastery(
      studentId: studentId,
      topicId: topicId,
      score: percentage,
    );
